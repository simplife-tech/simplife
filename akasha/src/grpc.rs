use std::{pin::Pin, task::{Poll, Context}, borrow::Cow};

use hyper::Body;
use opentelemetry::{global, propagation::{Injector, Extractor}, trace::{TracerProvider, Tracer, TraceContextExt, Status}};
use tonic::{body::BoxBody, metadata::{MetadataMap, KeyRef}};
    ...
    fn call(&mut self, req: hyper::Request<Body>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            *req.headers_mut() = metadata.into_headers();
            ...
            if status_code >= 400 {
                oc.span().set_status(Status::Error { description: Cow::from("ERROR") })
            }
            Ok(response)
        })
    }
